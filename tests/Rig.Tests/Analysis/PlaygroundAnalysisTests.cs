...
public sealed class PlaygroundAnalysisTests
{
    ...
    public async Task Entry_point_effects_playground_tracks_entrypoints_and_effects()
    {
        ...
        result.Effects.ShouldContain(effect =>
            ...
            && effect.Observations.Any(observation =>
                observation.Type == "looped_effect" && observation.Context == "foreach"
            )
        );
        ...
        result.Effects.ShouldContain(effect =>
            ...
            && effect.Resource == "billing.example/invoices/{teamId}"
            && effect.Observations.Any(observation =>
                observation.Type == "parallel_fanout" && observation.Context == "Task.WhenAll"
            )
        );
        ...
        result.Effects.ShouldContain(effect =>
            ...
            && effect.Operation == "read"
            && effect.Observations.Any(observation =>
                observation.Type == "parallel_fanout" && observation.Context == "Parallel.ForEach"
            )
        );
        ...
        result.InvocationObservations.ShouldContain(observation =>
            observation.ContainingMethodSymbol.Contains(
        ...
        result.DiRegistrations.ShouldContain(registration =>
            registration.ServiceType.Contains(
                "EntryPointEffects.Api.Services.TeamWorkflow",
                StringComparison.Ordinal
            )
            && registration.Lifetime == "scoped"
            && registration.Reason == "msdi_addscoped"
            && registration.Evidence.Contains(
                "project=EntryPointEffects.Api",
                StringComparison.Ordinal
            )
        );
        ...
        result.DiRegistrations.ShouldContain(registration =>
            registration.ServiceType.Contains(
                "EntryPointEffects.Api.Services.ITeamRepository",
                StringComparison.Ordinal
            )
            && registration.ImplementationType != null
            && registration.ImplementationType.Contains(
                "EntryPointEffects.Api.Services.TeamRepository",
                StringComparison.Ordinal
            )
        );
        ...
        result.DiRegistrations.ShouldContain(registration =>
            ...
            && registration.ImplementationType != null
            && registration.ImplementationType.Contains(
                "EntryPointEffects.Api.Services.BillingClient",
                StringComparison.Ordinal
            )
        );

        var minApiGetGraph = result
            .CallGraphs.Where(graph => graph.EntryPoint == "minapi GET /minapi/teams/{id}")
        ...
        symbols.ShouldContain(symbol =>
            symbol.Contains(
                "global::EntryPointEffects.Api.Services.TeamWorkflow.LoadTeamSummaryAsync",
                StringComparison.Ordinal
            )
        );
        symbols.ShouldContain(symbol =>
            symbol.Contains(
                "global::EntryPointEffects.Api.Services.TeamWorkflow.ObserveDynamicBoundary",
                StringComparison.Ordinal
            )
        );
        symbols.ShouldContain(symbol =>
            symbol.Contains(
                "global::EntryPointEffects.Api.Services.BillingClient.LoadInvoiceAsync",
                StringComparison.Ordinal
            )
        );
        symbols.ShouldContain(symbol =>
            symbol.Contains(
                "global::EntryPointEffects.Api.Services.BillingClient.LoadInvoicesAsync",
                StringComparison.Ordinal
            )
        );
        ...
        minApiGetGraph.Nodes.ShouldContain(node =>
            ...
            && node.Effects.Any(effect =>
                effect.Provider == "redis" && effect.Resource == "team:{relatedTeamId}"
            )
        );
        ...
        minApiGetGraph.Nodes.ShouldContain(node =>
            ...
            && node.Effects.Any(effect =>
                effect.Observations.Any(observation => observation.Type == "parallel_fanout")
            )
        );
        ...
        minApiGetGraph.Nodes.ShouldContain(node =>
            ...
            && node.BoundaryCalls.Any(call =>
                call.Kind == "external" && call.Method == "HttpClient.GetStringAsync"
            )
        );
        ...
        minApiGetGraph.Nodes.ShouldContain(node =>
            ...
            && node.BoundaryCalls.Any(call =>
                call.Kind == "unresolved" && call.Reason == "unresolved_call_target"
            )
        );
        ...
        var selfCycleGraph = result.CallGraphs.Single(graph =>
            graph.EntryPoint == "minapi GET /minapi/cycles/self"
        );
        selfCycleGraph
        ...
        var mutualCycleGraph = result.CallGraphs.Single(graph =>
            graph.EntryPoint == "minapi GET /minapi/cycles/mutual"
        );
        var mutualCycle = mutualCycleGraph.Cycles.ShouldHaveSingleItem().Path;
        mutualCycle.Count.ShouldBe(3);
        mutualCycle.ShouldContain(symbol =>
            symbol.Contains("CycleFixture.MutualA", StringComparison.Ordinal)
        );
        mutualCycle.ShouldContain(symbol =>
            symbol.Contains("CycleFixture.MutualB", StringComparison.Ordinal)
        );
        mutualCycle.First().ShouldBe(mutualCycle.Last());

        var threeStepCycleGraph = result.CallGraphs.Single(graph =>
            graph.EntryPoint == "minapi GET /minapi/cycles/three-step"
        );
        var threeStepCycle = threeStepCycleGraph.Cycles.ShouldHaveSingleItem().Path;
        threeStepCycle.Count.ShouldBe(4);
        threeStepCycle.ShouldContain(symbol =>
            symbol.Contains("CycleFixture.ThreeStepA", StringComparison.Ordinal)
        );
        threeStepCycle.ShouldContain(symbol =>
            symbol.Contains("CycleFixture.ThreeStepB", StringComparison.Ordinal)
        );
        threeStepCycle.ShouldContain(symbol =>
            symbol.Contains("CycleFixture.ThreeStepC", StringComparison.Ordinal)
        );
    ...
    public async Task Eshop_grpc_services_are_entrypoints_and_reach_redis_effects()
    {
        ...
        result.EntryPoints.ShouldContain(entryPoint =>
            entryPoint.Kind == "grpc"
            && entryPoint.DisplayName.Contains(
    ...
    public async Task Eshop_background_workers_and_event_handlers_are_entrypoints()
    {
        ...
        result.EntryPoints.ShouldContain(entryPoint =>
            ...
            && entryPoint.DisplayName.Contains(
                "GracePeriodManagerService.ExecuteAsync",
                StringComparison.Ordinal
            )
        );
        result.EntryPoints.ShouldContain(entryPoint =>
        ...
        gracePeriodGraph.Nodes.ShouldContain(node =>
            node.Effects.Any(effect =>
                effect.Provider == "eventbus" && effect.Operation == "publish"
            )
        );
        gracePeriodGraph.Nodes.ShouldContain(node =>
            node.Effects.Any(effect =>
                effect.Provider == "rabbitmq" && effect.Operation == "channel_open"
            )
        );
        gracePeriodGraph.Nodes.ShouldContain(node =>
            node.Effects.Any(effect =>
                effect.Provider == "rabbitmq" && effect.Operation == "declare_exchange"
            )
        );
        gracePeriodGraph.Nodes.ShouldContain(node =>
            node.Effects.Any(effect =>
                effect.Provider == "db_connection" && effect.Operation == "open"
            )
        );
        gracePeriodGraph.Nodes.ShouldContain(node =>
            node.Effects.Any(effect =>
                effect.Provider == "db_reader" && effect.Operation == "row_read"
            )
        );
        gracePeriodGraph.Nodes.ShouldContain(node =>
            node.Effects.Any(effect =>
                effect.Provider == "resilience" && effect.Operation == "execute"
            )
        );
