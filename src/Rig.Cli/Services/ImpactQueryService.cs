...
// via ImpactCommand.DiffAsync (the single compute path shared by CLI + web). Internal (not public like the
// other services) because the artifact/diff types are internal to ImpactCommand; the Web mapper — same
...
internal static class ImpactQueryService
{
    ...
    )
    {
        ...
        return await ImpactCommand.DiffAsync(
            headContext: context,
            ws: ws,
            baseRef: baseRef,
            headRef: headRef,
