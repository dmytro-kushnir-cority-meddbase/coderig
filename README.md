...
| `rig tree <pat> [--view paths\|full\|effects\|summary\|hazards] [--async] [--only/--exclude <p,..>] [--raw] [--maxdepth <n>]` | Call tree from an entry point. Default (`--view paths`) prunes to effect-reaching paths; `--view effects` lists only effectful methods; `--view hazards` marks pattern hazards (race_window/dual_write/…) inline + a summary section; `--raw` bypasses render rules |
