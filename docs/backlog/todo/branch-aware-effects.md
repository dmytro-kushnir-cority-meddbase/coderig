...
- **Open algorithmic work**: `GuardsOf` is still the delete-test (MB–GB; oracle-only). Guard-SET *predicates* need a
  post-dominator-tree + dominance-frontier rewrite before they touch extraction. The headline spine/guarded BOOLEAN
  render needs only must-run (already fast), so M3 can ship the partition without guard predicates.
