...
- **The reachability "delete-test" is O(V²·E) with per-query HashSet+Stack churn — catastrophic** (½ GB / 128 ms for one
  258-block method). Retained ONLY as the differential test ORACLE. Guard-SETS still run on it (`GuardsOf`), so the full
  guard-set computation is NOT yet on the hot path.
- **Decision:** the headline render (spine vs guarded *boolean*) needs only must-run → ships on the fast tree. Guard-set
  *predicates* (the richer M3 layer) wait for a **post-dominator tree + dominance-frontier** rewrite of `GuardsOf` (same
  CHK machinery on the reversed CFG), which the benchmark shows is mandatory before guard-sets touch extraction.
- **BenchmarkDotNet blocker** (separate infra fix): the BDN-autogenerated project lacks `IsPackable=false`, so it pulls
  `Meziantou.Analyzer` (a `Directory.Build.props` global `PackageReference` gated on `IsPackable`) with no CPM
  `PackageVersion` → `NU1010`. Affects ALL benchmarks here, not just CFG. `ControlDependenceBenchmarks` is committed and
  builds; it'll produce rigorous numbers once BDN+CPM is sorted.
