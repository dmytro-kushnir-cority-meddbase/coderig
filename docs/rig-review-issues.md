...
### Render backlog — propagate concrete generic types into tree labels (2026-06-15)
Tree nodes render generics with placeholder params (`QueryPipeline<T, U>.Enumerate`) because the label uses only the DocID open form (`PrettyGenericName(ShortName(node.SymbolId))`). But the CONCRETE instantiation is already computed + threaded along the path for dispatch narrowing — `FactPathFinder.Successors` carries `OutReceiver` (the propagated concrete receiver type, e.g. `QueryPipeline<PersonDataFieldDefinition, DefinitionAndRangeDto>`) and `OutBinding` (the accumulated concrete type-arg set from each edge's mined `TypeArguments`). **Proposal:** thread the reaching edge's `ReceiverType` onto `TraceNode` in `BuildTree`; in the formatter, when the node's declaring generic type matches the concrete reaching receiver, substitute its args for the `<T,U>` placeholders → `QueryPipeline<PersonDataFieldDefinition, DefinitionAndRangeDto>.Enumerate` (use `OutBinding` for generic-method args). Caveats: (1) a node renders once (first BFS path), so it shows THAT path's instantiation — a node reached with multiple instantiations shows one (consider a `(+N)` hint); (2) it's a `TraceNode` plumbing change (new field from the reaching edge + a substitution branch), wants its own test pass. Moderate, self-contained, real readability win; data already present.
