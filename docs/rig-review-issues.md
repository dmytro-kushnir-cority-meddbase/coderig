...
> **#3 permission:assert surface (2026-06-15 survey).** One dominant family: static `CertificateEntity.(Has|Assert)(Account|Chamber|Any)?Right(...)` (144+ hits/75 files), disambiguated from the polluting LLBLGen entity-nav `AssertRight()` by the presence of a `Rights.*`/`Rights.Wrapper`/`IRightsWrapper` argument. The right NAME is already extractable via **`resource:argument_name`** (`Rights.Person.CanViewCaseNames`) — rules-only for the v1 detector. Two nuances: bitwise-OR composites (`A | B`, a BinaryExpression) → `argument_name` returns null (needs set-extraction); some args are variables flowing from a param (name = param symbol, needs dataflow). Secondary clean surfaces: `[Authorize(Roles=Roles.X)]` attributes (EnterpriseApi, ~68, const-string name) and an isolated WebDav `RequirePermission(PermissionKinds)` model — each its own rule. **Implication: a v1 `permission:assert` provider on CertificateEntity with `resource:argument_name` is rules-only** (composite-OR + facade methods = refinements).
