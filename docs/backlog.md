...
## FR-7 cache-coherence — mechanism shipped (OFF by default), needs a generated-ORM filter before useful

**STATUS (2026-06-23): the detector MECHANISM is built, tested, committed, and OFF by default** (fires only when a
repo supplies a `cacheCoherence` rule). **NOT calibrated-clean — do NOT enable on MedDBase until refined.**

- **What shipped:** deriver `FactCacheCoherenceDeriver` (commit `0cf27585`) + the `cacheCoherence` rule schema +
  `DeriveCommand` wiring (next commit). A reach-join: a BULK write (`UpdateMulti`/`DeleteMulti`/
  `UpdateEntitiesDirectly`) to a cached entity X whose forward closure (`FactPathFinder.Reaches` from the write's
  caller) reaches no `XCache` invalidation (`Remove`/`RemoveKey`/`Clear`/`Invalidate`). Confidence `medium`
  (heuristic name-pairing + v1 forward/depth-capped reach). Off-by-default: no builtin `cacheCoherence` rule.
- **Calibration (MedDBase full store): 48 candidates, ALL in generated LLBLGen `CollectionClasses`**
  (`<X>Collection.UpdateMultiManyToOne`/`DeleteMultiManyToOne` — generated cascade helpers), 2/entity → 100%
  noise, zero real app-code findings surfaced. Generated ORM plumbing dominates. (Consistent with the
  "generated ≠ noise" lesson: index everything, but THIS detector must filter generated *enclosing* sites.)
- **Refinement (next unit):** add an `excludeEnclosing` gate to the deriver/rule — skip bulk-writes whose
  ENCLOSING method is generated ORM plumbing (declaring namespace ends `.CollectionClasses` / `.DaoClasses`, or
  a generated `<auto-generated>` file). Re-run `derive`, triage the REMAINING app-code candidates (the real
  targets — import/bulk paths like #4199). Watch the 3 deriver limits: substring-seed → `ReachesFromEachSeed`;
  `InvalidationReachable` O(mutations×edges) perf; `Reaches maxDepth=20` deep-invalidation FP.
- **Calibration rule data** (re-add to MedDBase `rig.rules.json` once refined): `cachedEntities` = the ~36
