...
externally. Completed / shipped / superseded items are archived in
[backlog-shipped.md](backlog-shipped.md) to keep this file forward-looking.
...
- **`cache_coherence` (+ `event_cycle`) are NOT in the per-EP `tree --view hazards` view** — they're
  whole-program graph-property derivations, surfaced only as `rig derive` hazard rows. So "run the FR-7
  detector scoped to an entry point" is impossible today; you reverse from the anchor with
  `callers <site> --entrypoints`. The natural feature: filter these findings to anchors reachable from `<EP>`
